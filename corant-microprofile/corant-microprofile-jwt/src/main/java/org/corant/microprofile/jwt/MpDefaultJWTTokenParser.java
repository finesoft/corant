 ...
 *
 * @author bingo 下午4:32:38
 *
 */
public class MpDefaultJWTTokenParser {
  ...
  public JwtContext parse(final String token, final JWTAuthContextInfo authContextInfo)
    ...
    try {
      JwtConsumerBuilder builder =
          new JwtConsumerBuilder().setRequireExpirationTime().setSkipDefaultAudienceValidation();
      select(ErrorCodeValidator.class).forEach(ecv -> {
        builder.registerValidator(ecv);// register black list tokenId
      });
  ...
  protected VerificationKeyResolver getKeyResolver(JWTAuthContextInfo authContextInfo)
  ...
  private void checkNameClaims(JwtContext jwtContext) throws InvalidJwtException {
    JwtClaims claimsSet = jwtContext.getJwtClaims();
    final boolean hasPrincipalClaim =
        Stream.of(Claims.sub.name(), Claims.upn.name(), Claims.preferred_username.name())
            .map(claimsSet::getClaimValue).anyMatch(Objects::nonNull);

    if (!hasPrincipalClaim) {
      throw new InvalidJwtException("No claim exists in sub, upn or preferred_username",
          new ArrayList<>(), jwtContext);
    }
  }

  private Object findClaimValue(String claimPath, Map<String, Object> claimsMap, String[] pathArray,
      int step) {
  ...
  private List<String> findGroups(JWTAuthContextInfo authContextInfo, JwtClaims claimsSet) {
    if (authContextInfo.getGroupsPath() != null) {
      ...
      Object claimValue = findClaimValue(authContextInfo.getGroupsPath(), claimsSet.getClaimsMap(),
          pathSegments, 0);
  ...
  private String findSubject(JWTAuthContextInfo authContextInfo, JwtClaims claimsSet) {
    if (authContextInfo.getSubjectPath() != null) {
      ...
      Object claimValue = findClaimValue(authContextInfo.getSubjectPath(), claimsSet.getClaimsMap(),
          pathSegments, 0);
      if (claimValue instanceof String) {
        return (String) claimValue;
      } else {
        logger.warnf("Claim value at the path %s is not a String",
            authContextInfo.getSubjectPath());
      }
    }
