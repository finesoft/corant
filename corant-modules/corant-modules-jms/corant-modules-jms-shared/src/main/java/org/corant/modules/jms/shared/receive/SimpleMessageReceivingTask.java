 ...
 */
public class SimpleMessageReceivingTask
  ...
  protected static final Logger logger =
      Logger.getLogger(SimpleMessageReceivingTask.class.getName());
  ...
  public SimpleMessageReceivingTask(MessageReceivingMetaData metaData) {
  ...
  public SimpleMessageReceivingTask(MessageReceivingMetaData metaData,
    ...
    logger.log(Level.FINE, () -> format("Create message receive task for %s.", metaData));
  }
    ...
    while (true) {
      ...
      if (!isInProgress() || t > 128) {
        logger.log(Level.INFO,
            () -> format("Cancel message receiving task try times %s, %s.", t, meta));
        return cancellation.compareAndSet(false, true);
      } else {
        logger.log(Level.INFO,
            () -> format("Waiting for message receiving task completed, %s.", meta));
  ...
  protected void postRun() {
    try {
      ...
      logger.log(Level.SEVERE, e, () -> format("The execution status occurred error, %s.", meta));
    }
  }
  ...
  protected boolean preRun() {
    if (!CDIs.isEnabled()) {
      logger.log(Level.SEVERE,
          () -> format("The execution can't run because the CDI not enabled, %s.", meta));
      return false;
    }
    if (state == STATE_BRK) {
  ...
  protected void stateRun() {
    resetMonitors();
    state = STATE_RUN;
    logger.log(Level.INFO, () -> format("The execution enters running mode, [%s]!", meta));
  }

  protected void stateTry() {
    resetMonitors();
    state = STATE_TRY;
    logger.log(Level.INFO, () -> format("The execution enters trying mode, [%s]!", meta));
  }
