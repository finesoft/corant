...
public class DefaultSqlQueryExecutor implements SqlQueryExecutor {
  ...
  public <T> T get(String sql, Class<T> resultClass, Object... args) throws SQLException {
    if (Map.class.isAssignableFrom(resultClass)) {
      Object obj = args.length > 0 ? getRunner().query(sql, new MapHandler(), args)
          : getRunner().query(sql, new MapHandler());
      return obj == null ? null : (T) obj;
    } else {
      return args.length > 0 ? getRunner().query(sql, new BeanHandler<>(resultClass), args)
          : getRunner().query(sql, new BeanHandler<>(resultClass));
    }
  }
  ...
  public <T> List<T> select(String sql, Class<T> resultClass, Object... args) throws SQLException {
    if (Map.class.isAssignableFrom(resultClass)) {
      ...
      List tmp = args.length > 0 ? getRunner().query(sql, new MapListHandler(), args)
          : getRunner().query(sql, new MapListHandler());
      return tmp == null ? new ArrayList<>() : tmp;
    } else {
      List<T> tmp =
          args.length > 0 ? getRunner().query(sql, new BeanListHandler<>(resultClass), args)
              : getRunner().query(sql, new BeanListHandler<>(resultClass));
      return tmp == null ? new ArrayList<>() : tmp;
    }
  }
